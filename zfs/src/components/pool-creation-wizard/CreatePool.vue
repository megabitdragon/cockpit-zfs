<template>
	<Modal :isOpen="showWizard" :marginTop="'mt-28'" :width="'w-3/5'" :minWidth="'min-w-3/5'">
		<template v-slot:title>
			<!-- navigation tabs for create pool wizard -->
			<WizardTabs :navigationItems="navigation" :currentNavigationItem="currentNavigationItem"
				:navigationCallback="navigationCallback" :show="show" />
		</template>
		<template v-slot:content>
			<!-- actual content for create pool wizard -->
			<PoolConfig ref="poolConfiguration" :navCallback="next" :currentNavigationItem="currentNavigationItem!"
				 :tag="navTag" idKey="pool-config" class="mx-4" />
		</template>
			...
			<div class="w-full grid grid-rows-2">
				...
				<div class="button-group-row w-full justify-between row-start-2">
					<div class="button-group-row mt-2">
						<button id="cancel" class="btn btn-danger object-left justify-start h-fit"
							@click="showWizard = false">Cancel</button>
						<div v-if="navTag == 'virtual-devices'" class="justify-self-start flex flex-row">
							<div class="button-group-row">
								<button v-if="poolConfig.vdevs.length > 0" id="add-vdev-btn"
									class="btn btn-primary object-left justify-start mr-4 h-fit w-full"
									@click="poolConfiguration.addVDev()">Add VDev</button>
							</div>

							<div class="flex flex-row">
								<label :for="'forcefully-create-pool'"
									class="mt-2 mr-2 block text-sm font-medium leading-6 text-default">Forcefully
									Create</label>
								<Switch v-model="poolConfig.properties.forceCreate"
									:class="[poolConfig.properties.forceCreate ? 'bg-primary' : 'bg-accent', 'mt-2 relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-slate-600 focus:ring-offset-2']">
									<span class="sr-only">Use setting</span>
									<span
										:class="[poolConfig.properties.forceCreate ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none relative inline-block h-5 w-5 transform rounded-full bg-default shadow ring-0 transition duration-200 ease-in-out']">
										<span
											:class="[poolConfig.properties.forceCreate ? 'opacity-0 duration-100 ease-out' : 'opacity-100 duration-200 ease-in', 'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity']"
											aria-hidden="true">
										...
										<span
											:class="[poolConfig.properties.forceCreate ? 'opacity-100 duration-200 ease-in' : 'opacity-0 duration-100 ease-out', 'absolute inset-0 flex h-full w-full items-center justify-center transition-opacity']"
											aria-hidden="true">
					...
					<div class="button-group-row mt-2">
						<button v-if="!end" id="back" class="btn btn-secondary object-left justify-start h-fit"
							@click="prev">Back</button>
						<button v-if="!end" id="next" class="btn btn-primary object-right justify-end h-fit"
							@click="next">Next</button>
						<!-- only show finish button if currently on the final tab -->
						<button v-if="end && !finishPressed" id="finish"
							class="btn btn-primary object-right justify-end h-fit"
							@click="finishBtn(newPoolData)">Finish</button>
						<button disabled v-if="end && finishPressed" id="finish" type="button"
