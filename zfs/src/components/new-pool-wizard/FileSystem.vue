  ...
  <div>
    ...
    <div class="relative flex items-start">
      <div class="text-sm mr-3">
        <label :for="getIdKey('create-filesystem')" class="font-medium text-gray-900">Create a File System?</label>
      </div>
      <div class="flex h-6 items-center">
        <input :id="getIdKey('create-filesystem')" v-model="poolConfig.createFileSystem" aria-describedby="create-filesystem" name="create-filesystem" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-slate-600 focus:ring-slate-600" />
      </div>
    </div>
      ...
      <div v-if="fileSystemConfig.encrypted">
        <!-- Passphrase (Text) -->
          <div>
            ...
            <input :id="getIdKey('passphrase')" type="password" v-model="fileSystemConfig.passphrase" name="passphrase" class="mt-1 block w-full rounded-md border-0 py-1.5 px-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-slate-300 sm:text-sm sm:leading-6" placeholder="Passphrase" />
          </div>
        <!-- Confirm Passphrase (Text) -->
        <div>
          <label :for="getIdKey('passphrase-confirm')" class="mt-1 block text-sm font-medium leading-6 text-gray-900">Confirm Passphrase</label>
          <input :id="getIdKey('passphrase-confirm')" type="password" name="passphrase-confirm" class="mt-1 block w-full rounded-md border-0 py-1.5 px-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-slate-300 sm:text-sm sm:leading-6" placeholder="Confirm Passphrase" />
        </div>
        <!-- Cipher (Select) -->
        <div>
