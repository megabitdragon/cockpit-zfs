<template>
  <div v-for="(vDev, vDevIdx) in configuration.vdevs" :key="vDevIdx">
    <fieldset>
      ...
      <div><span><p>{{ checkedDisks }}</p></span></div>
      ...
      <div>
        <br/>
        <button id="add-vdev" class="btn btn-primary object-right justify-end" @click="addVDev(false)">Add VDev</button>  
        <br/>
      </div>
...
const vDevDisks = ref<Disk[]>([]);


// watch(() => configuration.vdevs, () => {
//   claimDisks();
// }, {immediate: true, deep: true});

watch(checkedDisks, (newValue) => {
  claimDisks(newValue);
});

function claimDisks(selectedDisks: Disk[]) {
  vDevDisks.value = [];

  selectedDisks.forEach((selectedDisk) => {
    const index = availDisks.findIndex((disk) => disk.name === selectedDisk.name);
    if (index !== -1) {
      vDevDisks.value.push(...availDisks.splice(index, 1));
    }
  });
}


function addVDev(isFirst: boolean) {
  isPrimary.value = isFirst;
  configuration.vdevs.push(newVDev);
  if(isPrimary.value == false) {
    showRemoveButton.value = true;
  }
}

// function claimDisks() {
//   vDevDisks.value.push(...checkedDisks.value);
//   availDisks.splice(0, availDisks.length, ...availDisks.filter(disk => {
//     return !checkedDisks.value.some(checkedDisk => checkedDisk === disk);
//   }));
// }

function removeVDev(index: number) {
  configuration.vdevs = configuration.vdevs.filter((_, idx) => idx !== index) ?? [];
  // vDevs?.pop;
  // configuration!.vdevs = vDevs!;
}

console.log(checkedDisks);
console.log(vDevDisks);
// console.log(configuration);
// console.log(vDevs);

if (configuration.vdevs.length < 1) addVDev(true);
