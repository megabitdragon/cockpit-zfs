							...
							<MenuItems @click.stop class="absolute right-0 overflow-y-scroll z-10 w-[30rem] max-h-[40rem] origin-top-right rounded-md bg-default shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
								...
								<MenuItem as="div" v-for="notification in notificationStore.notifications" :key="notification.id" v-slot="{ active }">
									<div class="flex items-start gap-3" v-if="notification.event === 'scrub_finish'">
										...
										<div>
											<CheckCircleIcon 
												v-if="notification.errors === 'No' || notification.errors === null" 
										...
										<div class="w-full">
											...
											:class="{
												'text-green-500': notification.errors === 'No' || notification.errors ===null,
												'text-yellow-500': notification.errors !== null && notification.errors !== 'No'
											}"
											>
										...
										<p class="text-xl font-semibold" :class="{
											...
											'text-red-600': notification.state === 'FAULTED' ||  notification.state === 'REMOVED',
										}">
										...
										<p class="text-sm pl-4 text-white-600">
											...
											'text-green-500': notification.state === 'ONLINE',
											'text-yellow-500': notification.state === 'DEGRADED',
											'text-red-500': notification.state === 'FAULTED' || notification.state === 'REMOVED',
											}">
											{{ notification.state }}
											</span>
										</p>
										...
										<p v-if="notification.error !== 'None' && notification.error !== 'null'" class="text-sm pl-4 text-red-500">
											<strong>Error:</strong> {{ notification.error }}
										</p>

										<!-- Show Description if Exists -->
										<p v-if="notification.description !== 'None'" class="text-sm pl-4 text-white-500">
