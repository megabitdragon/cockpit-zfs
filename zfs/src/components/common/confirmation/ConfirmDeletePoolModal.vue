	...
	<Modal :isOpen="showDeleteConfirm" @close="showDeleteConfirm = false" :marginTop="'mt-60'" :width="'w-96'" :minWidth="'min-w-96'">
        ...
        <template v-slot:content>
            <div class="grid grid-flow-row mt-3 justify-items-center gap-1">
                <p class="text-default row-start-1">Are you sure you wish to <span class="text-danger">destroy</span> <b>{{ props.poolName }}</b>?</p>
                <div v-if="hasChildren" class="text-danger font-medium grid grid-rows-3 row-span-3 row-start-2 justify-items-center">
                    ...
                    <p class="text-danger row-start-2">Pool <b>{{ props.poolName }}</b> has children.</p>
