<template>
	<Modal :isOpen="showDetachDiskModal" @close="showDetachDiskModal = false" :marginTop="'mt-60'" :width="'w-96'" :minWidth="'min-w-96'">
        <template v-slot:title>
            <legend class="flex justify-center">Detach Disk</legend>
        </template>
        <template v-slot:content>
            <div class="grid grid-flow-row mt-3 justify-items-center gap-1">
                <p class="text-default row-start-1">Are you sure you wish to detach <b>{{ props.diskName }}</b> from <b>{{props.poolName}}</b>?</p>
            </div>
        </template>
        <template v-slot:footer>
            <div class="w-full grid grid-rows-1">
                <div class="button-group-row mt-2 justify-between">
                    ...
                    <button v-if="!detaching" @click="confirmDetach" :id="getIdKey('confirm-detach-yes-B')" name="detach-button-yes-B" class="mt-1 btn btn-primary object-right justify-end h-fit">Detach</button>
...
interface ConfirmDetachDiskProps {
    ...
    poolName: string;
    diskName: string;
    confirmDetach: ConfirmationCallback;
}
