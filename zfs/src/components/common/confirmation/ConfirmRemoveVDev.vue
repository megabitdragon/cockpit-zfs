        ...
        <template v-slot:footer>
            <div class="w-full grid grid-rows-1">
                <div class="button-group-row mt-2 justify-between">
                    ...
                    <button v-if="!removing" @click="confirmRemove" :id="getIdKey('confirm-remove-yes-B')" name="remove-button-yes-B" class="mt-1 btn btn-primary object-right justify-end h-fit">Remove</button>
...
interface ConfirmRemoveVDevModalProps {
    ...
    poolName: string;
    vDevName: string;
    confirmRemove: ConfirmationCallback;
}
