...
import { ref, watch, reactive, WatchStopHandle } from 'vue';
...
interface Notification {
    ...
    title: string;
    body: string;
    level: 'info' | 'warning' | 'error' | 'success' | 'denied';
    timeout: number;
 ...
 */
const constructNotification = (title: string, body: string, level: Notification['level'], timeout = 10000): Notification => {
    const actions: any[] = [];
     ...
     */
    obj.addAction = (text: string, callback: () => void) => {
        obj.actions.push({
            text,
            callback: () => {
                callback();
 ...
 * 
 */
const showNotificationObj = (notificationObj: Notification) => {
