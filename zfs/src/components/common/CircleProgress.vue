<template>
    <div v-if="props.justCircle">
        <div class="flex items-center justify-center -m-6 overflow-visible bg-default rounded-full">
            ...
            <span :class="['absolute', 'text-2xl', props.fillColor]">{{props.percentage}}%</span>
        </div>
    </div>
    <div v-if="!props.justCircle" class="flex items-center flex-wrap max-w-sm px-10 bg-default rounded-2xl h-20">
        <div class="flex items-center justify-center -m-6 overflow-visible bg-default rounded-full">
            <svg class="w-32 h-32 transform translate-x-1 translate-y-1" aria-hidden="true">
                <circle :id="getIdKey('circle-empty')" class="text-gray-300 dark:text-gray-500" :stroke-width="props.strokeWidth" stroke="currentColor" fill="transparent" :r="props.radius" :cx="props.coordX" :cy="props.coordY" />
                <circle :id="getIdKey('circle-filled')" :class="props.fillColor" :stroke-width="props.strokeWidth" :stroke-dasharray="props.strokeDashArr" :stroke-dashoffset="filledAmount" stroke-linecap="round" stroke="currentColor" fill="transparent" r="50" cx="60" cy="60" />
