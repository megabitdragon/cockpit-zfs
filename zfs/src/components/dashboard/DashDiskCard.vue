							...
							<MenuItems class="absolute right-0 z-10 -mt-1 w-max origin-top-left rounded-md bg-accent shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
								<div class="py-1">
									...
									<MenuItem v-slot="{ active }">
										<a href="#" @click.stop="clearPartAndRefreshDisks(props.disk)" :class="[active ? 'bg-danger text-default' : 'text-muted', 'block px-4 py-2 text-sm']">Clear Partitions (wipefs -a)</a>
									</MenuItem>
								</div>
							</MenuItems>
						</transition>
