				...
				<div class="button-group-row w-full justify-between row-start-2">
					<div class="button-group-row mt-2">
                        ...
                        <div class="flex flex-row">
                            <label :for="getIdKey('force-add-vdev')" class="mt-2 mr-2 block text-sm font-medium leading-6 text-default">Forcefully Add</label>
                            <Switch v-model="newVDev.forceAdd" :id="getIdKey('force-add-vdev')" :class="[newVDev.forceAdd! ? 'bg-primary' : 'bg-accent', 'mt-2 relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-slate-600 focus:ring-offset-2']">
                                <span class="sr-only">Use setting</span>
					...
					<div class="button-group-row mt-2">
                        <button v-if="!adding" id="add-vdev-btn" class="btn btn-primary object-right justify-end mr-4 h-fit w-full" @click="addVDevBtn">Add VDev</button>
