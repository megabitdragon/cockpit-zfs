       ...
       <template v-slot:content>
            <div>
                ...
                <div>
                    ...
                    <select :id="getIdKey('parent-filesystem')" name="parent-filesystem" v-model="fileSystemConfig.parentFS" class="bg-default mt-1 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-default ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-slate-600 sm:text-sm sm:leading-6">
                ...
                <div>
                    ...
                    <input :id="getIdKey('filesystem-name')" type="name" name="pool-name" v-model="fileSystemConfig.name" class="input-textlike mt-1 block w-full rounded-md border-0 py-1.5 px-1.5 text-default bg-default shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-muted focus:ring-2 focus:ring-inset focus:ring-slate-300 sm:text-sm sm:leading-6" placeholder="File System Name" />
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            <input v-model="fileSystemConfig.properties.quota.raw" :id="getIdKey('fs-quota-amount')" name="fs-quota-slider" type="range" min="0" max="1000" value="0" step="1" class="text-default bg-default mt-1 w-3/4 h-2 bg-accent rounded-lg appearance-none cursor-pointer ">
                            <input v-model="fileSystemConfig.properties.quota.raw" :id="getIdKey('fs-quota-num')" name="fs-quota-num"  type="number" value="0" class="text-default bg-default mt-1 w-1/4 block rounded-md border-0 py-1.5 px-1.5 text-default shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-muted focus:ring-2 focus:ring-inset focus:ring-slate-300 sm:text-sm sm:leading-6"/>
                            <select v-model="fileSystemConfig.properties.quota.size" :id="getIdKey('fs-quota-size')" name="fs-quota-size" class="bg-default block sm:col-span-1 rounded-md border-0 py-1.5 pl-3 pr-10 text-default ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-slate-600 sm:text-sm sm:leading-6">
...
const fileSystemConfig : FileSystemData = {
    parentFS: '',
    name: '',
    id: '',
    mountpoint: '',
...
const nameCheck = () => {
	...
	} else if (fileSystemConfig.name.match(/^[0-9]/) ) {
		result = false;
		nameFeedback.value = 'Name cannot begin with numbers.';
	} else if (fileSystemConfig.name.match(/^[.]/ )) {
		result = false;
		nameFeedback.value = 'Name cannot begin with a period (.).';
	} else if (fileSystemConfig.name.match(/^[_]/)) {
		result = false;
	...
	} else if (fileSystemConfig.name.match(/^c[0-9]|^log|^mirror|^raidz|^raidz1|^raidz2|^raidz3|^spare/)) {
		result = false;
		nameFeedback.value = 'Name cannot begin with a reserved name.';
	} else if (!fileSystemConfig.name.match(/^[a-zA-Z0-9_.:-]*$/)) {
		result = false;
