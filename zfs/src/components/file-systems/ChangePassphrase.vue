<template>
   <Modal :isOpen="showChangePassphrase" @close="showChangePassphrase = false" :marginTop="'mt-28'" :width="'w-3/5'" :minWidth="'min-w-3/5'">
        ...
        <template v-slot:content>
            <div>
                <div>
                    <label :for="getIdKey('passphrase')" class="mt-1 block text-sm font-medium leading-6 text-default">New Passphrase</label>
        ...
        <template v-slot:footer>
            <div class="w-full grid grid-rows-2">
                ...
                <div class="button-group-row w-full row-start-2 justify-between mt-2">
                    <button id="cancel" class="mt-1 btn btn-danger object-left justify-start h-fit" @click="showChangePassphrase = false">Cancel</button>
                    <button v-if="!changing" @click="changeBtn()" :id="getIdKey('change-passphrase-btn')" name="change-passphrase-btn" class="mt-1 btn btn-primary object-right justify-end h-fit">Change</button>
                    <button disabled v-if="changing" :id="getIdKey('change-pass-spinner')" type="button" class="btn btn-danger object-right justify-end">
